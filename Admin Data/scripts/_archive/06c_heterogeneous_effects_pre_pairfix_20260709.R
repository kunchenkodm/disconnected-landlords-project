...
#          regression:  D_y ~ 1 + D_rooms + D_floor  (numerically identical
#          treatment coefficient, half the rows, no subclass FE).
#
...
# Deduplicate by UPRN so the pair join is 1:1
n_dup <- sum(duplicated(dat$uprn))
if (n_dup > 0L) {
  message(sprintf("  NOTE: dropping %s duplicate-UPRN rows before the pair join.",
                  formatC(n_dup, big.mark = ",")))
...
ctrl_pool <- dat[!is.na(treat_for_profit) & treat_for_profit == 0L]
...
build_pair_diffs <- function(config) {
  ...
  # Join covariates + outcomes (dat is unique by uprn)
  pr <- dat[md, on = "uprn", nomatch = 0]
  tv <- pr[[config$var]]
  pr[, tv := tv]
  pr <- pr[!is.na(tv)]

  # Keep only intact pairs: exactly 1 treated + 1 control per subclass
  cnt <- pr[, .(n = .N, n_t = sum(tv == 1L)), by = subclass]
  good <- cnt[n == 2L & n_t == 1L, subclass]
  n_broken <- nrow(cnt) - length(good)
  broken_share <- if (nrow(cnt) > 0L) n_broken / nrow(cnt) else NA_real_
  if (!is.na(broken_share) && broken_share > BROKEN_PAIR_WARN_SHARE) {
    message(sprintf("  WARNING [%s]: %.1f%% of subclasses broken (dropped %d of %d) — investigate.",
                    config$short_id, broken_share * 100, n_broken, nrow(cnt)))
  }
  pr <- pr[subclass %in% good]
  if (nrow(pr) == 0L) {
    log_error(config$short_id, "pairs", "no intact pairs after integrity filter")
    return(NULL)
  }
...
for (config in treatment_metadata) {
  ...
  for (oc in hte_outcomes) {
    ...
    # Equivalence check vs 06's headline coefficient
    if (oc == "current_energy_efficiency" && !is.null(ref_coefs)) {
      rc <- ref_coefs[treatment_short_id == tsid, ref_coef]
      if (length(rc) == 1L && !is.na(rc)) {
        message(sprintf("  Equivalence check: pair-diff ATT = %.6f vs results_table = %.6f (diff %.2e)",
                        att_coef, rc, abs(att_coef - rc)))
      }
    }
